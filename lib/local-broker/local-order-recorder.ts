...
/**
 * The single fill-decision computation — pure, synchronous, no network, no
 * class state. Exported standalone (not just a private method) so
 * `/api/orders/confirm` can show a confirmation screen's "estimated fill"
 * using the EXACT same calculation `LocalOnlyOrderRecorder.submitLocalOrder`
 * will actually use at execute time, rather than a second reimplementation
 * that could quietly drift from it. The only difference between a preview
 * call and a real submission is which caller invokes this function and
 * whether the result gets persisted — the math is identical either way.
 */
export function previewFill(
  request: LocalOrderRequest,
  reference: ReferenceQuote,
  config: LocalFillConfig = DEFAULT_LOCAL_FILL_CONFIG
): LocalOrderResult {
  const submittedAt = new Date().toISOString();
  const localOrderId = `local-${crypto.randomUUID()}`;

  const { effectivePrice, slippageApplied } = computeEffectivePrice(request.side, config.fillModel, config.slippagePct, reference);
 ...
 * class — every result is computed synchronously via `previewFill`. This is
 * the ONLY place in the codebase order placement happens;
 * lib/order-executor/executor.ts's `recorder` parameter is typed as this
 * concrete class, not an interface anything Alpaca-backed could also
 * satisfy.
 */
export class LocalOnlyOrderRecorder {
  ...
  submitLocalOrder(request: LocalOrderRequest, reference: ReferenceQuote): LocalOrderResult {
    return previewFill(request, reference, this.config);
  }
}
